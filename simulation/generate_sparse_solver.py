...
def generate():
    ...
    for i,j in positions:
        if original[i,j]: lines.append(f'    m[{slots[i,j]}]=a[{rows[i]*N+columns[j]}];')
    ...
    print(json.dumps({k:v for k,v in meta.items() if k not in ('rows','columns')},indent=2))
